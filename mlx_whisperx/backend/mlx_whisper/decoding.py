...
class BeamSearchDecoder(TokenDecoder):
    ...
    def __init__(
        ...
        self.max_candidates = max(beam_size, round(beam_size * self.patience))
    ...
    def update(
        ...
        for audio_idx in range(n_audio):
            ...
            for beam_idx in range(start, end):
                ...
                for token_id in top_indices:
                    ...
                    sequence = prefix + (token_id,)
                    score = float(sum_logprobs_list[beam_idx]) + float(row[token_id])
            ...
            for sequence in sorted(scores, key=scores.get, reverse=True):
                ...
                next_sum_logprobs.append(score)
            ...
            if saved != self.beam_size:
                raise RuntimeError(
                    f"beam search produced {saved} active beams, expected {self.beam_size}"
                )
        ...
        max_len = max(len(sequence) for sequence in next_tokens)
        padded_tokens = [
            list(sequence) + [self.eot] * (max_len - len(sequence))
            for sequence in next_tokens
        ]
    ...
    def finalize(self, tokens: mx.array, sum_logprobs: mx.array):
        ...
        max_candidates = max(len(group) for group in all_tokens)
        max_len = max(len(sequence) for group in all_tokens for sequence in group)
        padded_tokens = []
        padded_scores = []
        ...
        return mx.array(padded_tokens), mx.array(padded_scores)
